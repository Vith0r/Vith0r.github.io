const z = "astro:before-preparation"
  , J = "astro:after-preparation"
  , Q = "astro:before-swap"
  , Z = "astro:after-swap"
  , ee = e => document.dispatchEvent(new Event(e));
class $ extends Event {
    from;
    to;
    direction;
...
class ne extends $ {
    ...
    swap;
    constructor(t, n, r) {
        super(Q, void 0, t.from, t.to, t.direction, t.navigationType, t.sourceElement, t.info, t.newDocument),
...
async function re(e, t, n, r, o, a, l, h) {
    const s = new te(e,t,n,r,o,a,window.document,l,h);
    return document.dispatchEvent(s) && (await s.loader(),
    s.defaultPrevented || (ee(J),
    s.navigationType !== "traverse" && D({
        scrollX,
        scrollY
    }))),
    s
...
const q = e => document.dispatchEvent(new Event(e))
  ...
  , V = "data-astro-transition-fallback";
let P, g = 0;
history.state ? (g = history.state.index,
scrollTo({
...
const X = (e, t, n, r) => {
    ...
    a = !0),
    r ? scrollTo(r.scrollX, r.scrollY) : (e.hash ? (history.scrollRestoration = "auto",
    location.href = e.href) : a || scrollTo({
        left: 0,
        top: 0,
        behavior: "instant"
    }),
...
async function I(e, t, n, r) {
    const o = (i, u) => {
        const m = i.getAttribute(w)
          , p = m && u.head.querySelector(`[${w}="${m}"]`);
        if (p)
            return p;
        if (i.matches("link[rel=stylesheet]")) {
            const b = i.getAttribute("href");
            return u.head.querySelector(`link[rel=stylesheet][href="${b}"]`)
        }
        return null
    }
      , a = () => {
      ...
      , h = i => {
        ...
        [...i.newDocument.documentElement.attributes, ...m].forEach( ({name: c, value: f}) => u.setAttribute(c, f));
        for (const c of document.scripts)
            for (const f of i.newDocument.scripts)
                if (!c.src && c.textContent === f.textContent || c.src && c.type === f.type && c.src === f.src) {
                    f.dataset.astroExec = "";
                    break
                }
        for (const c of Array.from(document.head.children)) {
            const f = o(c, i.newDocument);
            f ? f.remove() : c.remove()
        }
        ...
        for (const c of p.querySelectorAll(`[${w}]`)) {
            const f = c.getAttribute(w)
              , S = document.querySelector(`[${w}="${f}"]`);
            S && S.replaceWith(c)
        }
        l(b)
    }
    ...
    async function s(i) {
        ...
        document.documentElement.setAttribute(V, i);
        const b = document.getAnimations().filter(c => !m.includes(c) && !u(c));
        return Promise.all(b.map(c => c.finished))
    }
    if (!T)
        document.documentElement.setAttribute(U, e.direction),
        r === "animate" && await s("old");
    else
        throw new DOMException("Transition was skipped");
    const d = await oe(e, y, h);
    X(d.to, d.from, t, n),
    q(Z),
    r === "animate" && !T && s("new").then( () => _())
}
async function Y(e, t, n, r, o) {
    ...
    const a = o ? "traverse" : r.history === "replace" ? "replace" : "push";
    if (a !== "traverse" && D({
        scrollX,
        scrollY
    }),
    ...
    const l = await re(t, n, e, a, r.sourceElement, r.info, r.formData, h);
    ...
    async function h(s) {
        const d = s.to.href
          , i = {};
        s.formData && (i.method = "POST",
        i.body = s.formData);
        const u = await ae(d, i);
        if (u === null) {
            s.preventDefault();
            return
        }
        if (u.redirected && (s.to = new URL(u.redirected)),
        P ??= new DOMParser,
        s.newDocument = P.parseFromString(u.html, u.mediaType),
        ...
        const m = le(s.newDocument);
        m.length && await Promise.all(m)
    }
    if (T = !1,
    R)
        y = document.startViewTransition(async () => await I(l, r, o));
    else {
        const s = (async () => {
...
async function O(e, t) {
    await Y("forward", A, new URL(e,location.href), t ?? {})
}
function ue(e) {
    if (!x() && e.state) {
        location.reload();
...
;
{
    (R || W() !== "none") && (A = new URL(location.href),
    addEventListener("popstate", ue),
    addEventListener("load", C),
    "onscrollend"in window ? addEventListener("scrollend", M) : addEventListener("scroll", se(M, 350), {
        passive: !0
    }));
...
function de() {
    for (const e of ["touchstart", "mousedown"])
        document.body.addEventListener(e, t => {
            E(t.target, "tap") && k(t.target.href, {
                with: "fetch",
...
function pe(e, t) {
    ...
    try {
        ...
        return location.origin === n.origin && (location.pathname !== n.pathname || location.search !== n.search) && !B.has(e)
    } catch {}
    return !1
}
function E(e, t) {
    ...
    return n === "false" ? !1 : t === "tap" && (n != null || L) && j() ? !0 : n == null && L || n === "" ? t === K : n === t
}
function j() {
...
(R || ye() !== "none") && (document.addEventListener("click", e => {
    let t = e.target;
    ...
    N(t) || t.hasAttribute("download") || !t.href || n && n !== "_self" || o !== location.origin || e.button !== 0 || e.metaKey || e.ctrlKey || e.altKey || e.shiftKey || e.defaultPrevented || (e.preventDefault(),
    O(r, {
...
document.addEventListener("submit", e => {
    let t = e.target;
    if (t.tagName !== "FORM" || N(t))
        return;
    const n = t
      , r = e.submitter
      , o = new FormData(n,r);
    let a = r?.getAttribute("formaction") ?? n.action ?? location.pathname;
    const l = r?.getAttribute("formmethod") ?? n.method;
    if (l === "dialog")
        return;
    const h = {
        sourceElement: r ?? n
